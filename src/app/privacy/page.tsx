...
const CONTENT = {
  id: {
    ...
    updated: "Terakhir diperbarui: 2 Agustus 2026",
    login: "Masuk",
    sections: [
      ...
      { title: "4. Berbagi Data dengan Pihak Ketiga", body: "Kami berbagi data dengan pihak ketiga terbatas berikut: Groq Inc. (pemrosesan teks AI saat pertanyaan dijawab; Groq menyatakan tidak memakai data API pelanggan untuk melatih modelnya), Google LLC (seluruh isi dokumen yang Anda unggah dikirim ke Gemini API untuk dibuatkan indeks pencarian, pertanyaan yang diajukan dikirim untuk pencocokan, dan saat Groq mencapai batas pemakaian potongan dokumen yang relevan dikirim ke Gemini API untuk menyusun jawaban; akun kami saat ini berada pada tier gratis, dan ketentuan Google untuk tier tersebut mengizinkan Google memakai konten untuk meningkatkan layanannya — hubungi kami jika kebijakan dokumen Anda tidak mengizinkan hal ini), Midtrans (pemrosesan pembayaran, berlisensi Bank Indonesia), Neon Inc. (penyimpanan database terenkripsi). Kami tidak berbagi data dengan pihak lain tanpa persetujuan eksplisit Anda." },
      { title: "5. Hak Anda (sesuai UU PDP No. 27/2022)", body: "Anda berhak: mengakses data Anda, mengoreksi data yang tidak akurat, menghapus akun dan seluruh data, menarik persetujuan pemrosesan data, mengajukan keberatan atas pemrosesan data. Untuk permintaan tersebut, hubungi: hello@intellibaseai.com" },
      ...
      { title: "9. Kontak", body: "Pertanyaan tentang privasi dan perlindungan data: hello@intellibaseai.com" },
    ],
  },
  en: {
    ...
    sections: [
      ...
      { title: "4. Sharing Data with Third Parties", body: "We share data with the following limited third parties: Groq Inc. (AI text processing when a question is answered; Groq states that it does not use customer API data to train its models), Google LLC (the full contents of documents you upload are sent to the Gemini API to build a search index, questions asked are sent for matching, and when Groq is at its usage limit the relevant document excerpts are sent to the Gemini API to compose the answer; our account is currently on the free tier, and Google's terms for that tier allow Google to use content to improve their services — contact us if your document policy does not permit this), Midtrans (payment processing, licensed by Bank Indonesia), Neon Inc. (encrypted database storage). We do not share your data with any other parties without your explicit consent." },
      { title: "5. Your Rights (pursuant to Indonesian Personal Data Protection Law No. 27/2022)", body: "You have the right to: access your data, correct inaccurate data, delete your account and all associated data, withdraw consent for data processing, and object to data processing. To exercise these rights, contact: hello@intellibaseai.com" },
      ...
      { title: "9. Contact", body: "Questions about privacy and data protection: hello@intellibaseai.com" },
    ],
  },
};
