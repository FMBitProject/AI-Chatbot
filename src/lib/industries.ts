...
// The vertical the homepage is written for. Since the hospital repositioning the
// landing page's hero, problem, and how-it-works copy all speak to hospitals and
// clinics directly, so this is no longer a band promoted out of a strip — it is
// the page. Everything else is "also fits" and lives on /industri.
export const PRIMARY_INDUSTRY_KEY = "rumah-sakit";
...
// Every vertical except the one the homepage is written for: what the bottom
// row of the landing page names and what /industri lists. Derived here rather
// than filtered at each call site so the two surfaces can never disagree about
// which industries count as "other".
export const OTHER_INDUSTRIES = INDUSTRIES.filter(
  (i) => i.key !== PRIMARY_INDUSTRY_KEY,
);
