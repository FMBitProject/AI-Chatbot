      ...
      <nav className="border-b border-hairline bg-background/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <LanguageSwitcher />
            <Link href="/blog" className="text-sm text-stone-500 hover:text-stone-800 font-medium hidden md:block">{T.blog}</Link>
            <Link href="/roi" className="text-sm text-stone-500 hover:text-stone-800 font-medium hidden md:block">{T.roi}</Link>
            <Link href="/pricing" className="text-sm text-stone-500 hover:text-stone-800 font-medium hidden md:block">{T.price}</Link>
            <Link href="/login"><Button variant="ghost" size="sm" className="hidden sm:inline-flex">{T.login}</Button></Link>
            <Link href="/register"><Button size="sm" className="bg-teal-700 hover:bg-teal-800 text-xs sm:text-sm px-3 sm:px-4">{T.start}</Button></Link>
          </div>
        </div>
      </nav>
